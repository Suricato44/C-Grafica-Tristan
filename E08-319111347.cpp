...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//---- Sigue Luz Coche
		glm::vec3 PosicionFaro = glm::vec3(2.05f, 1.05f, 3.2+mainWindow.getmuevex());
		glm::vec3 Direccion = glm::vec3(0.0f, 0.0f, 1.0f);
		glm::vec3 lowerLight = PosicionFaro;
		lowerLight.y -= 0.3f;
		spotLights[2].SetFlash(lowerLight, Direccion);

		//---- Sigue Luz Helicoptero
		glm::vec3 PosicionFaroHeli = glm::vec3(mainWindow.getmueveheli(), 4.0f, 6.0f);
		glm::vec3 DireccionHeli = glm::vec3(0.0f, -1.0f, 0.0f);
		spotLights[1].SetFlash(PosicionFaroHeli, DireccionHeli);


		glm::vec3 lowerLight2 = camera.getCameraPosition();
		lowerLight2.y -= 0.3f;
		spotLights[0].SetFlash(lowerLight2, camera.getCameraDirection());
		...
		model = glm::translate(model, glm::vec3(mainWindow.getmueveheli(), 0.0f, 0.0f));
